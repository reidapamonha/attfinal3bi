...
--
-- Host: 127.0.0.1
-- Tempo de geração: 30/09/2024 às 20:20
...
--
-- Acionadores `tb_pagamento`
--
DELIMITER $$
CREATE TRIGGER `after_insert_tb_pagamento` AFTER INSERT ON `tb_pagamento` FOR EACH ROW BEGIN
    -- Check if the payment amount exceeds or equals 1000
    IF NEW.Valor >= 1000 THEN
        -- Automatically update the status to 'completed'
        UPDATE tb_pagamento
        SET status = 'completed'
        WHERE id = NEW.ID_Pagamento;
    END IF;
END
$$
DELIMITER ;
